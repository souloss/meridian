...
# M3 diff/share/todo + CLI: breaking diff, snapshot share, breaking todos, push, CLI exit codes.
smoke-m3-diff-cli:
	sh ./scripts/smoke.sh --m3-diff-cli
...
cli-contract-tests:
	vfox exec golang@1.27.1 -- go test -count=1 ./internal/command -run '^TestCli|^TestDiffExitCodes|^TestHTTPExitCodes'
...
# M4 search + dependency graph: index and search 10,000 items under the perf budget.
perf-search-graph:
	mkdir -p artifacts/spikes
	SPIKE_REPORT_PATH=artifacts/spikes/perf-search-graph.json vfox exec golang@1.27.1 -- go run ./cmd/perf-search-graph
