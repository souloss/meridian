...
> 版本：v1.0（定稿）
> 技术栈：Go（后端）+ Nuxt 4（前端）+ PostgreSQL（存储）
...
- 服务级授权（`service_members`）：可把某个服务的 owner/maintainer/viewer 授予具体用户或团队
   ...
   └── Repository 仓库          —— Git 地址 + 凭证 + 分支策略
          └── Service 服务      —— 仓库内的逻辑服务（系统的最小纳管单元）
                 └── Asset 资产（kind + name 的身份槽位）
                        ├── AssetSource 资产源 1..N ─┐  每个源产出/维护一个层
                        ├── Layer 层 1..N ◄──────────┘  base 层 + overlay 层
                        │      └── LayerRevision 层修订（不可变，内容寻址）
                        └── AssetVersion 资产版本（一次确定性合并的不可变快照）
                               └── AssetItem 资产条目（扁平索引：端点/事件/表/字段…）
...
| **Repository** | 被纳管的代码仓库。含 URL、凭证引用、默认分支、扫描路径、忽略规则 | 租户内「URL + 分支」唯一 |
...
| **AssetSource** | 「一个层怎么来」的配置：mode（builtin/command/push/manual/ai）+ 路径/命令 + 目标层 | 一个源恰好绑定一个层；一个资产可有多个源 |
...
| **LayerRevision** | 层内容的不可变修订（内容哈希去重）。repo 层修订绑定 git commit；manual 层修订绑定一次 UI 编辑；ai 层修订绑定一次生成任务 | 修订不可变；层的"当前修订"指针可移动（即可回滚） |
| **AssetVersion** | 合并产物的不可变快照：记录参与合并的各层 revision、合并顺序、合并引擎版本、结果哈希、溯源清单 | 输入（层修订集合 + 顺序）相同 ⇒ 不产生新版本 |
...
**生命周期**：`draft → published → deprecated → retired`，Service 与 Asset 共用同一状态机。
...
- F1.1 本地账号密码登录（argon2id 哈希）+ 会话管理（HttpOnly Cookie / JWT）
...
- F1.7 服务级授权：`service_members` 表，支持在单个服务上授予 owner / maintainer / viewer
- F1.8 可见性：服务可设 `private`（仅授权可见）/ `internal`（租户内登录可见）/ `public`（匿名可见）
- F1.9 API Key / PAT：绑定租户（或平台级 bot）；支持作用域（`asset:push`、`asset:read`、`job:run`）、过期时间、最后使用时间、可撤销
...
- F2.1 凭证类型：`ssh_key`（ed25519 优先，兼容 RSA，可选 passphrase）/ `http_token`（用户名 + PAT，GitHub / GitLab / Gitee / Bitbucket 通用）/ `none`（公开仓库）
- F2.2 加密存储：主密钥（`MASTER_KEY`）→ HKDF 派生每凭证子密钥 → AES-256-GCM；DB 只存密文
- F2.3 掩码展示：只展示指纹、类型、创建人、最后使用时间；明文私钥仅创建时可见一次
- F2.4 归属与共享：凭证归属租户，默认创建者私有，可显式共享给租户内团队/全体；`platform_admin` 可标记 `global` 凭证供所有租户使用
- F2.5 known_hosts：SSH 首次连接记录 host key 指纹，可配 `accept-new` / 手动录入
...
- F4.5 服务删除：软删除 + 关联资产级联归档
- F4.6 **资产源配置（核心）** —— 每个服务可配 0..N 个资产源，每个源定义**一个层**怎么来：

  | 配置项 | 说明 |
  |---|---|
  | `asset_kind` / `asset_name` | 归属的资产身份；资产不存在时自动创建。name 不填时从路径或文件名推导 |
  ...
  | `command` | `command` 模式的外部命令（生成器入口） |
  | `order` | overlay 应用顺序（仅 overlay 层） |
  | `timeout` / `enabled` | 单源超时；可临时停用而不删配置 |

  - 约束：一个资产至多一个 base 源；无 base 源时资产以「空 base」存在（AI 冷启动场景，F13.1）
  - glob 展开为多个资产：`api/*/openapi.yaml` 命中 3 个文件时自动展开成 3 个 Asset（名字取目录名，可配命名模板），每个各得一个独立 base 层
...
resolve（拉代码/读缓存）
  ...
  → extract（各 AssetSource 产出各自的 LayerRevision）
...
- F5.1 **Producer 模式**：
  ...
  | `command` | 平台在工作区快照内执行外部命令产出资产文件；任何能命令行调用的生成器（swag、buf、springdoc、自建 CLI）均可接入 | ✅ 扩展点 |
  | `push` | 外部系统通过 `POST /assets/push` 或 `assetctl push` 推送 | ✅（CLI v1 提供） |
...
- F5.2 **层修订（LayerRevision）**：
  - 任何源的一次成功产出 = 一个新修订；内容哈希去重，未变不新建
  - repo 层修订绑定 git commit；manual 层修订绑定操作用户；ai 层修订绑定生成任务 id + 模型标识 + prompt 摘要
  - 层可独立回滚到任意历史修订（回滚 = 移动当前修订指针，产生新 AssetVersion）
- F5.3 **合并引擎（核心）**：
  - 输入：base 当前修订 + 启用的 overlay 层按 order 排序的当前修订序列
...
- F5.5 **AssetVersion**：
  - 每次 merge 输入集合变化 ⇒ 新 AssetVersion；记录 `{layer_id → revision_id}` 清单 + 引擎版本 + 结果哈希
  - 版本号自动语义化递增（按 breaking / minor / patch），支持手动指定与打标签
  - 状态机 `draft → published → deprecated → retired`；旧版本永久可访问；「当前 / 最新」可切换
  - 发布策略：base 来自 repo 且无待审 overlay ⇒ 可自动 publish（可配）；包含未审批的 ai / third_party 修订 ⇒ 强制停留 draft（F13.4）
- F5.6 归一化与下载：
...
- F6.1 **ViewDef**：
  ```ts
  interface ViewDef {
    id: string                      // 'swagger-ui' | 'diff' | 'dep-graph' | ...
    name: string
    input: ViewInputSpec            // 输入契约，见 F6.2
    mount: 'component' | 'iframe' | 'external'
    enabled: boolean
    order: number
    optionsSchema?: JSONSchema      // 视图自身参数（渲染成设置面板 + 进分享链接）
  }
  ```
- F6.2 **ViewInputSpec —— 视图输入契约（四模式）**：
  ```ts
  type ViewInputSpec =
    | { mode: 'single';                    // 单资产单版本：详情类视图
        kinds: AssetKindId[] | '*' }
    | { mode: 'versions';                  // 同一资产的 N 个版本：DIFF / 时间线
        kinds: AssetKindId[] | '*';
        minDocs: number; maxDocs: number } // diff=2..2, changelog=2..50
    | { mode: 'collection';                // 多个同 kind 资产（可跨服务/仓库/分支）
        kinds: AssetKindId[] | '*';
        sameKind: true; maxDocs: number }
    | { mode: 'scope';                     // 范围聚合：不点名资产，给一个作用域
        scope: 'service' | 'system_group' | 'tenant';
        kinds: AssetKindId[] | '*' }
  ```
  ...
  - 分支/版本表达：`versions` 模式的每个 doc 可指向「已入库版本」或「某分支最新」（解析为该分支最近一次成功采集的 AssetVersion）；`collection` 模式允许同一资产的不同分支作为两个 doc——覆盖"同仓库不同版本/分支对比"与"跨仓库对比"两类场景
...
- F7.2 **Diff 器按 kind 插件化**：
  - `openapi`：结构化变更模型（libopenapi/what-changed + 自定义分级）。层级 `Document → Path → Operation → Parameter / RequestBody / Response / Schema / Security`；类型 `added / removed / modified`；级别 `breaking / risky / non-breaking / informational`
...
- F11.2 CLI `assetctl`：`push`（CI 推送资产/层）、`diff --fail-on breaking`（门禁）、`list` / `sync` / `produce` / `ai-gen`（F13 参考实现）
...
- F12.3 备份：`pg_dump` 定时备份 + 保留策略；blob 存储（层内容/工作区）随备份策略覆盖
...
> 平台**不内置 AI 模型**：AI 调用通过 `command` 型 producer 或外部系统 push 完成，平台负责编排、状态机与审批。官方提供参考实现 `assetctl ai-gen`（调用 OpenAI 兼容 API 分析代码生成 openapi.yaml）作为可选组件。AI 调用的成本、限流、鉴权由外部工具负责。

- F13.1 **冷启动流程（P0）**：
  1. 接入仓库/分支/服务后，发现某服务无 `openapi` 资产（或 base 为空）
  2. 目录页/服务页展示「资产缺失」+「用 AI 生成」按钮；也可在采集管道配置为自动触发
  3. 平台创建 `ai` 模式 AssetSource（`layer_origin = ai_generated`），投递 `asset.ai_generate` 任务：在服务代码快照目录执行配置的 AI 命令，产出写入 `$OUTPUT_DIR`
  4. 产出通过 kind 校验器 → 新 LayerRevision（状态 `pending_review`）→ 通知有 `layer:approve` 权限的人
  5. 审核界面 = `layers` 视图 + 合并前后 diff 预览；**通过** ⇒ 修订生效、触发 merge、版本可发布；**驳回** ⇒ 修订归档留痕，可带意见重新生成
- F13.2 AI 层的角色：服务无任何 base 时，AI 层可作为 base（origin 仍为 ai_generated）；已有 repo base 时，AI 只能产出 overlay（如"为所有缺 description 的端点补描述"）
- F13.3 工程约束：幂等（content_hash 去重）、超时可配（默认 10min）、失败重试、token 用量透传字段（可空）；AI 修订永不自动 publish
- F13.4 **审批状态机**（对 `ai_generated` 与 `third_party` 层生效）：`pending_review → approved / rejected`；仅 approved 修订参与 merge；租户可配「信任模式」跳过审批（默认关闭）
...
| 安全 | 凭证静态加密；密钥不进日志/响应；CSRF；PAT 仅展示一次；分享链接签名 + 过期；AI command 受限目录执行、可禁网；依赖漏洞扫描进 CI |
...
| 部署 | 单二进制 + 内嵌前端（`embed`）；docker-compose（app + postgres）一条命令启动；另提供内嵌 PG 的单容器镜像作为体验模式 |
...
| D2 | 存储 = PostgreSQL（层/版本/provenance 用 JSONB，全局视图用递归 CTE，队列用 River） | docker-compose 部署；体验模式提供内嵌 PG 单容器镜像 |
...
| 里程碑 | 周期(参考) | 内容 | 验收标准 |
|---|---|---|---|
| **M0 地基** | 2-3 周 | 租户/权限/PAT、凭证、仓库录入与同步、River 队列、AssetKind 注册表与核心表结构 | 越权用例全绿；录入仓库完成 clone 与服务发现 |
| **M1 资产主链路** | 3-4 周 | openapi kind 全量：file-glob base 层 → merge（恒等）→ normalize → AssetVersion → swagger-ui / redoc / source / operations / items-table | 5 分钟内从录入仓库看到第一份文档；内容哈希去重生效 |
| **M2 层与 Overlay** | 3 周 | Layer / LayerRevision 模型、通用 Overlay 引擎、OpenAPI Overlay 1.0 方言、manual 层编辑 + 实时合并预览、layers 视图、provenance | 三层合并确定性用例通过；source 视图可标注来源层 |
| **M3 AI 闭环 + Diff** | 3 周 | F13 冷启动全流程（含 `assetctl ai-gen`）、审批状态机、openapi diff + breaking 规则、diff 视图 | 演示：接入无文档仓库 → AI 生成 → 审批 → 发布 → 与下一版 diff |
...
| **M5 协作与开放** | 3 周 | 订阅通知、breaking 确认待办、REST API 全覆盖、`assetctl push / diff --fail-on breaking`、出向 webhook | CI 门禁演示通过 |
...
```
asset_kinds        (id, content_format, capabilities, enabled)
assets             (tenant_id, service_id, kind, name, lifecycle)        UNIQUE(service_id, kind, name)
asset_sources      (asset_id, layer_id, mode, path, command, timeout, enabled, config_source)
layers             (asset_id, role, origin, "order", enabled, current_revision_id)
layer_revisions    (layer_id, content_hash, content_ref, status[active|pending_review|rejected|archived],
                    git_commit, created_by, ai_meta jsonb)
asset_versions     (asset_id, version, status, merge_engine_ver, layer_manifest jsonb,
                    merged_hash, provenance_ref)
asset_items        (tenant_id, asset_version_id, item_type, key, display jsonb,
                    search_text tsvector, provenance jsonb)
system_groups / system_group_members
view_defs          (id, input_spec jsonb, mount, enabled, "order", options_schema jsonb)
```
...
services:
  - name: order-service
    ...
    assets:
      ...
      - kind: dbschema
        name: main
        base: { mode: command, command: "atlas inspect --format json" }
```

### A.4 关键 API（示意）

```
POST /api/v1/t/{tenant}/assets/push                     # push 模式入口（PAT 作用域 asset:push）
POST /api/v1/t/{tenant}/assets/{id}/layers/{lid}/revisions   # 提交层修订（manual/third_party）
POST /api/v1/t/{tenant}/layer-revisions/{rid}/approve   # 审批通过
POST /api/v1/t/{tenant}/layer-revisions/{rid}/reject    # 审批驳回（带意见）
POST /api/v1/t/{tenant}/views/resolve                   # input descriptor → DocRef[]/ItemQuery（视图输入解析器）
GET  /api/v1/t/{tenant}/asset-versions/{vid}/provenance # 溯源清单
```
