...
| M0 Nuxt 控制面 | 部分完成 | 登录、租户壳、认证/租户守卫、仓库/凭据查询与创建、Job 查询/取消/重试/SSE、桌面/移动导航和表单负向校验已实现；平台级管理视图、编辑/删除工作流和完整 Smoke fixture 仍未闭环 | `16af457` |
...
最后稳定基线是 `16af457`。该提交完成时，Go 单测、竞态测试、`go vet`、数据库/HTTP 集成测试、契约校验、DDL/生成注释审计和 `git diff --check` 均已通过；提交后的生成无漂移检查随后通过。
...
- 前端验证已通过 `vfox exec nodejs@24.20.0 -- pnpm --dir web typecheck`、`pnpm generate`、`pnpm check:generated-docs`；Playwright CI 矩阵为 10 项（9 passed、1 个桌面移动抽屉用例按视口跳过），每个执行项目的 axe 违规均为 0；1440px Dashboard 与 393px Jobs 抽屉截图无横向溢出。
- 当前前端边界是平台级管理视图、资源编辑/删除工作流和完整 Smoke fixture 尚未接入；已实现的创建路径不会以禁用按钮或假成功占位。
...
1. 补齐 M0 Nuxt 平台级控制面、资源编辑/删除工作流，并为剩余写入路径增加契约/负向 E2E。
